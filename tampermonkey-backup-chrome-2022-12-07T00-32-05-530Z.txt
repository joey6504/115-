...
// @require      https://unpkg.com/underscore@1.12.0/underscore-min.js
// @require      https://unpkg.com/sweetalert2@11.3.0
// @require      https://unpkg.com/node-forge@0.10.0/dist/forge.min.js
// @require      https://unpkg.com/emoutils@2.0.0/dist/umd/emoutils.min.js
...
(function () {
    ...
    function getFileItemPreid(fileItem) {
        ...
        const r = new Promise((resolve, reject) => {
			  ...
			  console.log(fileItem.pickCode,test,tm);
			  ({data, key} = m115_encode(test, tm));
            GM_xmlhttpRequest({
                method: "POST",
                ...
                onload: function (r) {
                    if (r.status == 200) {
                        ...
                        if (download_info.state && download_info.data) {
                            try {
                                var json = m115_decode(download_info.data,key);
                                //console.log(json)
    ...
    function uploadFile(targetFolder, fileItem, uploadInfo) {
        ...
        let appVersion = "25.0.0.3";
        const secret = "Qclm8MGWUv59TnrR0XPg"

        let time = (new Date()).getTime();
        let ts = Math.floor(time / 1000);

        let uploadToken = md5(`${secret}${fileItem.sha1}${fileItem.size}${fileItem.preid}${uploadInfo.user_id}${ts}${md5(uploadInfo.user_id)}${appVersion}`)
